...
from mainion_hq import Ui_MainWindow
...
#        self.phase_modulation = PhaseModulation(self.ui)
        ...
        # hq added connects and start ups
    ...
    def need_to_crystallize(self):
        ...
        gs2[16] = 0 #set to open shutter for strong beam
        current = gs2
    ...
    def crystallized(self):
        ...
        gs2[16] = 1 #set to close shutter for strong beam
        current = gs2
        current = ionseq.fpga.bound(current)
        ionseq.fpga.fpgaseq = []
        ionseq.fpga.fpgaseq.append(SeqLine(gs[0].delay, gs[0].scanned, current.copy() ) )
...
#        filename = QtGui.QFileDialog.getSaveFileName(self, 'Open file', '~')
    ...
    def new_data_slot(self):
        ...
        if (self.simple_scan.isActive()):
            ...
            self.freq_scan.add_point(value, histogram, value2, fpga_data)
#            self.freq_scan.add_point2(value,histogram)
        elif(self.delay_scan.isActive()):
            self.delay_scan.add_point(value, histogram, value2, fpga_data)
