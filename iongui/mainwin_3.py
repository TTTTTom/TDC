...
#            self.phase_modulation = Modulation(self.ui)
        ...
        QtCore.QObject.connect(self.ui.actionAbout, QtCore.SIGNAL("triggered()"), self.about_slot)
    ...
    def about_slot(self):
        QtGui.QMessageBox.about(self, "Nothing makes sense", "Ions are violet, violets are blue, some ON bits are red, and some OFF bits too. When in doubt, sound the alarm.")

    def eom_gui_slot(self):
        QtGui.QMessageBox.about(self, "EOM", "End Of Message")
    #     self.iodine.show()
    ...
    def find_doppler_dds(self):
        dev = None
        for devidx in self.freq_scan.devices:
            if devidx.getName() == "DDSQO0012": # If this is a doppler cooling DDS
                dev = devidx
        if dev == None:
            print "Doppler cooling DDS DDSQO0012 is not on the list. WHY NOT ON THE LIST BRO"
        return dev
...
#        filename = QtGui.QFileDialog.getSaveFileName(self, 'Open file', '~')
    ...
    def new_data_delay_slot(self):
        if self.ui.TrigCam_checkBox.isChecked():
            self.camera.slot_snap_clicked()
            self.run_delay_timer.start(self.camera.exposure + 500)  #  500 us is the time to acquire the image from the camera
    ...
    ### Trying new new_data_slot with add_points instead of add_point s.t. no hanging will occur when data stream back too fast
    # def new_data_slot(self):
    #     # Update histogram and calculate value to plot
    #     (value, histogram, value2, fpga_data) = self.ion_data.new_histogram(self.camera.get_intensity())
    #     # (value, histogram, value2, fpga_data) = self.ion_data.new_histogram()
